...
var v = (r, a, t) => a in r ? k(r, a, { enumerable: !0, configurable: !0, writable: !0, value: t }) : r[a] = t;
var b = (r, a, t) => v(r, typeof a != "symbol" ? a + "" : a, t);
const h = {
  isHex(r) {
  ...
  rgbToHex(r, a, t) {
    if (r > 255 || a > 255 || t > 255) throw "Invalid color component";
    return (r << 16 | a << 8 | t).toString(16);
  },
  // rgbToHsv(rgb: any) {
  // },
  ...
  hexToHsv(r) {
    const a = parseInt(r.substring(1, 3), 16) / 255, t = parseInt(r.substring(3, 5), 16) / 255, e = parseInt(r.substring(5, 7), 16) / 255, i = Math.max(a, t, e), n = Math.min(a, t, e);
    let s;
    if (i === n)
      s = 0;
    else if (i === a)
  ...
  hsvToHex(r) {
    let a, t, e, i = r.h / 360, n = r.s, s = r.v, c = Math.floor(i * 6), u = i * 6 - c, l = s * (1 - n), p = s * (1 - u * n), d = s * (1 - (1 - u) * n);
    switch (c % 6) {
      case 0:
        a = s, t = d, e = l;
        break;
  ...
  hsvToHsl(r) {
    let a = r.h / 360, t = r.s, e = r.v, i = (2 - t) * e / 2;
    return i != 0 && (i == 1 ? t = 0 : i < 0.5 ? t = t * e / (i * 2) : t = t * e / (2 - i * 2)), { h: a, s: t, l: i };
  }
}, C = (r) => {
  throw Error(r);
}, o = {
  assert: (r, a, t) => a.find((e) => e == r) || t || C(),
  toInt: (r, a) => parseInt(r, 10) || a
  // isTruthy: (val) => !!val,
...
class f extends HTMLElement {
  ...
  initOption() {
    this.option.sbSquare = this.hasAttribute("sb-square"), this.option.hue.picker.class = this.getAttribute("hue-picker-class") ?? "hue-picker", this.option.hue.picker.type = o.assert(this.getAttribute("hue-picker-type"), ["linear", "circular"], "linear"), this.option.hue.picker.direction = o.assert(this.getAttribute("hue-picker-direction"), ["horizontal", "vertical"], "horizontal"), this.option.hue.picker.length = o.toInt(this.getAttribute("hue-picker-length"), 200), this.option.hue.picker.thickness = o.toInt(this.getAttribute("hue-picker-thickness"), 10), this.option.hue.picker.radius = o.toInt(this.getAttribute("hue-picker-thickness"), 100), this.option.hue.indicator.class = this.getAttribute("hue-indicator-class") ?? "hue-indicator", this.option.hue.indicator.type = o.assert(this.getAttribute("hue-indicator-type"), ["circle", "rectangle"], "circle"), this.option.hue.indicator.size = o.toInt(this.getAttribute("hue-indicator-size"), 16), this.option.hue.indicator.border.color = this.getAttribute("hue-indicator-border-color") ?? "white", this.option.hue.indicator.border.thickness = o.toInt(this.getAttribute("hue-indicator-border-thickness"), 2), this.option.saturation.picker.class = this.getAttribute("saturation-picker-class") ?? "saturation-picker", this.option.saturation.picker.direction = o.assert(this.getAttribute("saturation-picker-direction"), ["horizontal", "vertical"], "horizontal"), this.option.saturation.picker.length = o.toInt(this.getAttribute("saturation-picker-length"), 200), this.option.saturation.picker.thickness = o.toInt(this.getAttribute("saturation-picker-thickness"), 10), this.option.saturation.indicator.class = this.getAttribute("saturation-indicator-class") ?? "saturation-indicator", this.option.saturation.indicator.type = o.assert(this.getAttribute("saturation-indicator-type"), ["circle", "rectangle"], "circle"), this.option.saturation.indicator.size = o.toInt(this.getAttribute("saturation-indicator-size"), 16), this.option.saturation.indicator.border.color = this.getAttribute("saturation-indicator-border-color") ?? "white", this.option.saturation.indicator.border.thickness = o.toInt(this.getAttribute("saturation-indicator-border-thickness"), 2), this.option.brightness.picker.class = this.getAttribute("brightness-picker-class") ?? "brightness-picker", this.option.brightness.picker.direction = o.assert(this.getAttribute("brightness-picker-direction"), ["horizontal", "vertical"], "horizontal"), this.option.brightness.picker.length = o.toInt(this.getAttribute("brightness-picker-length"), 200), this.option.brightness.picker.thickness = o.toInt(this.getAttribute("brightness-picker-thickness"), 10), this.option.brightness.indicator.class = this.getAttribute("brightness-indicator-class") ?? "brightness-indicator", this.option.brightness.indicator.type = o.assert(this.getAttribute("brightness-indicator-type"), ["circle"], "circle"), this.option.brightness.indicator.size = o.toInt(this.getAttribute("brightness-indicator-size"), 16), this.option.brightness.indicator.border.color = this.getAttribute("brightness-indicator-border-color") ?? "white", this.option.brightness.indicator.border.thickness = o.toInt(this.getAttribute("brightness-indicator-border-thickness"), 2);
  }
  initState() {
    if (h.isHex(this.value)) {
      const { h: t, s: e, v: i } = h.hexToHsv(this.value);
      this.state.hue = t, this.state.saturation = e, this.state.brightness = i, this.state.hueHex = h.hsvToHex({ h: t, s: 1, v: 1 }), this.state.huePosition = t / 360 * this.option.hue.picker.length, this.state.saturationPosition = e * this.option.saturation.picker.length;
  ...
  setHue(t, e) {
    ...
    const c = this.elems.hueCanvas.getContext("2d"), [u, l, p] = c.getImageData(n, s, 1, 1).data, d = "#" + ("000000" + h.rgbToHex(u, l, p)).slice(-6), { h: g } = h.hexToHsv(d);
  ...
  setSB(t, e) {
    ...
    const c = this.elems.sbCanvas.getContext("2d"), [u, l, p] = c.getImageData(n, s, 1, 1).data, d = "#" + ("000000" + h.rgbToHex(u, l, p)).slice(-6), { s: g, v: m } = h.hexToHsv(d);
  ...
  initHueContainer() {
    this.elems.hueContainer = document.createElement("div"), this.elems.hueContainer.classList.add(this.option.hue.picker.class), this.elems.hueContainer.style.position = "relative", this.elems.hueContainer.style.lineHeight = "0", this.appendChild(this.elems.hueContainer);
  }
  initSbContainer() {
    this.elems.sbContainer = document.createElement("div"), this.elems.sbContainer.style.position = "relative", this.elems.sbContainer.style.lineHeight = "0", this.appendChild(this.elems.sbContainer);
  }
  initSaturationContainer() {
    this.elems.saturationContainer = document.createElement("div"), this.elems.saturationContainer.classList.add(this.option.saturation.picker.class), this.elems.saturationContainer.style.position = "relative", this.elems.saturationContainer.style.lineHeight = "0", this.appendChild(this.elems.saturationContainer);
  }
  initBrightnessContainer() {
    this.elems.brightnessContainer = document.createElement("div"), this.elems.brightnessContainer.classList.add(this.option.brightness.picker.class), this.elems.brightnessContainer.style.position = "relative", this.elems.brightnessContainer.style.lineHeight = "0", this.appendChild(this.elems.brightnessContainer);
  }
  initContainer() {
    this.initHueContainer(), this.option.sbSquare ? this.initSbContainer() : (this.initSaturationContainer(), this.initBrightnessContainer());
  }
  initHueCanvas() {
  ...
  initSbCanvas() {
    this.elems.sbCanvas = document.createElement("canvas"), this.elems.sbCanvas.addEventListener("mousedown", (t) => {
  ...
  initSaturationCanvas() {
    this.elems.saturationCanvas = document.createElement("canvas"), this.elems.saturationCanvas.addEventListener("mousedown", (t) => {
  ...
  initBrightnessCanvas() {
    this.elems.brightnessCanvas = document.createElement("canvas"), this.elems.brightnessCanvas.addEventListener("mousedown", (t) => {
  ...
  initCanvas() {
    this.initHueCanvas(), this.option.sbSquare ? this.initSbCanvas() : (this.initSaturationCanvas(), this.initBrightnessCanvas());
  }
  initHueIndicator() {
  ...
  initSbIndicator() {
    this.elems.sbIndicator = document.createElement("div"), this.elems.sbIndicator.style.position = "absolute", this.elems.sbIndicator.style.boxSizing = "border-box", this.elems.sbIndicator.style.zIndex = "1", this.elems.sbIndicator.style.width = `${this.option.common.indicator.size}px`, this.elems.sbIndicator.style.height = `${this.option.common.indicator.size}px`, this.elems.sbIndicator.style.border = `${this.option.common.indicator.border.thickness}px solid ${this.option.common.indicator.border.color}`, this.elems.sbIndicator.style.borderRadius = "50%", this.elems.sbDragArea = document.createElement("div"), this.elems.sbDragArea.style.position = "fixed", this.elems.sbDragArea.style.width = "100%", this.elems.sbDragArea.style.height = "100%", this.elems.sbDragArea.style.top = "0", this.elems.sbDragArea.style.left = "0", this.elems.sbDragArea.style.display = "none", this.elems.sbIndicator.appendChild(this.elems.sbDragArea), this.drawSBIndicator(), this.elems.sbIndicator.addEventListener("mousedown", (t) => {
  ...
  initIndicators() {
    this.initHueIndicator(), this.option.sbSquare ? this.initSbIndicator() : (this.initSaturationIndicator(), this.initBrightnessIndicator());
  }
  drawHueIndicator() {
