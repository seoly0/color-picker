...
export class HSVPicker extends HTMLElement {
  ...
  initOption() {
    ...
    this.option.common.picker.class = this.getAttribute( 'picker-class' ) || 'hsv-picker'
    ...
    this.option.common.indicator.class = this.getAttribute( 'indicator-class' ) ?? 'hsv-indicator'
    ...
    this.option.hue.picker.class = this.getAttribute( 'hue-picker-class' ) || this.option.common.picker.class
    this.option.hue.picker.type = valueUtils.assert( this.getAttribute( 'hue-picker-type' ), [ 'linear', 'circular' ], 'linear' )
    this.option.hue.picker.direction = valueUtils.assert( this.getAttribute( 'hue-picker-direction' ), [ 'horizontal', 'vertical' ], this.option.common.picker.direction )
    this.option.hue.picker.length = valueUtils.toInt( this.getAttribute( 'hue-picker-length' ), this.option.common.picker.length )
    this.option.hue.picker.thickness = valueUtils.toInt( this.getAttribute( 'hue-picker-thickness' ), this.option.common.picker.thickness )
    this.option.hue.picker.radius = valueUtils.toInt( this.getAttribute( 'hue-picker-radius' ), 100 )

    this.option.hue.indicator.class = this.getAttribute( 'hue-indicator-class' ) || this.option.common.indicator.class
    this.option.hue.indicator.type = valueUtils.assert( this.getAttribute( 'hue-indicator-type' ), [ 'circle', 'rectangle' ], this.option.common.indicator.type )
    this.option.hue.indicator.size = valueUtils.toInt( this.getAttribute( 'hue-indicator-size' ), this.option.common.indicator.size )
    this.option.hue.indicator.border.color = this.getAttribute( 'hue-indicator-border-color' ) || this.option.common.indicator.border.color
    this.option.hue.indicator.border.thickness = valueUtils.toInt( this.getAttribute( 'hue-indicator-border-thickness' ), this.option.common.indicator.border.thickness )

    this.option.saturation.picker.class = this.getAttribute( 'saturation-picker-class' ) || this.option.common.picker.class
    this.option.saturation.picker.direction = valueUtils.assert( this.getAttribute( 'saturation-picker-direction' ), [ 'horizontal', 'vertical' ], this.option.common.picker.direction )
    this.option.saturation.picker.length = valueUtils.toInt( this.getAttribute( 'saturation-picker-length' ), this.option.common.picker.length )
    this.option.saturation.picker.thickness = valueUtils.toInt( this.getAttribute( 'saturation-picker-thickness' ), this.option.common.picker.thickness )

    this.option.saturation.indicator.class = this.getAttribute( 'saturation-indicator-class' ) || this.option.common.indicator.class
    this.option.saturation.indicator.type = valueUtils.assert( this.getAttribute( 'saturation-indicator-type' ), [ 'circle', 'rectangle' ], this.option.common.indicator.type )
    this.option.saturation.indicator.size = valueUtils.toInt( this.getAttribute( 'saturation-indicator-size' ), this.option.common.indicator.size )
    this.option.saturation.indicator.border.color = this.getAttribute( 'saturation-indicator-border-color' ) || this.option.common.indicator.border.color
    this.option.saturation.indicator.border.thickness = valueUtils.toInt( this.getAttribute( 'saturation-indicator-border-thickness' ), this.option.common.indicator.border.thickness )

    this.option.brightness.picker.class = this.getAttribute( 'brightness-picker-class' ) || this.option.common.picker.class
    this.option.brightness.picker.direction = valueUtils.assert( this.getAttribute( 'brightness-picker-direction' ), [ 'horizontal', 'vertical' ], this.option.common.picker.direction )
    this.option.brightness.picker.length = valueUtils.toInt( this.getAttribute( 'brightness-picker-length' ), this.option.common.picker.length )
    this.option.brightness.picker.thickness = valueUtils.toInt( this.getAttribute( 'brightness-picker-thickness' ), this.option.common.picker.thickness )

    this.option.brightness.indicator.class = this.getAttribute( 'brightness-indicator-class' ) || this.option.common.indicator.class
    this.option.brightness.indicator.type = valueUtils.assert( this.getAttribute( 'brightness-indicator-type' ), [ 'circle' ], this.option.common.indicator.type )
    this.option.brightness.indicator.size = valueUtils.toInt( this.getAttribute( 'brightness-indicator-size' ), this.option.common.indicator.size )
    this.option.brightness.indicator.border.color = this.getAttribute( 'brightness-indicator-border-color' ) ?? this.option.common.indicator.border.color
    this.option.brightness.indicator.border.thickness = valueUtils.toInt( this.getAttribute( 'brightness-indicator-border-thickness' ), this.option.common.indicator.border.thickness )

    this.option.selected.show = this.hasAttribute( 'show-selected' )
    this.option.selected.width = valueUtils.toInt( this.getAttribute( 'selected-width' ), 200 )
    this.option.selected.height = valueUtils.toInt( this.getAttribute( 'selected-height' ), 200 )
  }
  ...
  initHueIndicator() {
    ...
    this.elems.hueIndicator.addEventListener( 'mousedown', ( evt ) => {
      ...
      if ( this.elems.sbIndicator ) this.elems.sbIndicator.style.zIndex = '1'
      if ( this.elems.saturationIndicator ) this.elems.saturationIndicator.style.zIndex = '1'
      if ( this.elems.brightnessIndicator ) this.elems.brightnessIndicator.style.zIndex = '1'
    } )
  ...
  initSbIndicator() {
    ...
    this.elems.sbIndicator.addEventListener( 'mousedown', ( evt ) => {
      ...
      if ( this.elems.hueIndicator ) this.elems.hueIndicator.style.zIndex = '1'
      if ( this.elems.saturationIndicator ) this.elems.saturationIndicator.style.zIndex = '1'
      if ( this.elems.brightnessIndicator ) this.elems.brightnessIndicator.style.zIndex = '1'
    } )
  ...
  drawSaturationIndicator() {
    if ( this.option.saturation.picker.direction == 'horizontal' ) {
      ...
      this.elems.saturationIndicator!!.style.left = `${( this.option.saturation.picker.thickness - this.option.saturation.indicator.size ) / 2}px`
      this.elems.saturationIndicator!!.style.top = `${this.state.saturationPosition - this.option.saturation.indicator.size / 2}px`
    }
  ...
  drawBrightnessIndicator() {
    if ( this.option.saturation.picker.direction == 'horizontal' ) {
      ...
      this.elems.brightnessIndicator!!.style.left = `${( this.option.brightness.picker.thickness - this.option.brightness.indicator.size ) / 2}px`
      this.elems.brightnessIndicator!!.style.top = `${this.state.brightnessPosition - this.option.brightness.indicator.size / 2}px`
    }
  ...
  drawSaturationPicker() {
    ...
    // 수직
      ...
      this.elems.saturationCanvas!!.width = this.option.saturation.picker.thickness
      this.elems.saturationCanvas!!.height = this.option.saturation.picker.length

      const saturationCTX = this.elems.saturationCanvas!!.getContext( '2d', { willReadFrequently: true } )!!

      const saturationCTXGrad = saturationCTX.createLinearGradient( 0, 0, 0, this.option.saturation.picker.length )
      saturationCTXGrad.addColorStop( 0 / 1, colorUtils.hsvToHex( { h: this.state.hue, s: 0, v: this.state.brightness } ) )
      saturationCTXGrad.addColorStop( 1 / 1, colorUtils.hsvToHex( { h: this.state.hue, s: 1, v: this.state.brightness } ) )

      saturationCTX.fillStyle = saturationCTXGrad
      saturationCTX.fillRect( 0, 0, this.option.saturation.picker.thickness, this.option.saturation.picker.length )
    }
  }
  ...
  drawBrightnessPicker() {
    ...
    // 수직
      ...
      this.elems.brightnessCanvas!!.width = this.option.brightness.picker.thickness
      this.elems.brightnessCanvas!!.height = this.option.brightness.picker.length

      const brightnessCTX = this.elems.brightnessCanvas!!.getContext( '2d', { willReadFrequently: true } )!!

      const brightnessCTXGrad = brightnessCTX.createLinearGradient( 0, 0, 0, this.option.brightness.picker.length )
      brightnessCTXGrad.addColorStop( 0 / 1, colorUtils.hsvToHex( { h: this.state.hue, s: this.state.saturation, v: 0 } ) )
      brightnessCTXGrad.addColorStop( 1 / 1, colorUtils.hsvToHex( { h: this.state.hue, s: this.state.saturation, v: 1 } ) )

      brightnessCTX.fillStyle = brightnessCTXGrad
      brightnessCTX.fillRect( 0, 0, this.option.brightness.picker.thickness, this.option.brightness.picker.length )
    }
  }
